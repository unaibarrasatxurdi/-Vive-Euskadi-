...
<div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4 mt-5 justify-content-center divUserId" :id="this.userId">
  ...
  <div v-for="(item, index) in paginated('resultado')" :key="index" class="d-flex justify-content-center mb-2">
      ...
      <router-link exact-active-class="active" :to="{path: '/busqueda/'+$route.params.textoBusqueda+'/plan/'+item.documentName, query: { plan: userId}}" aria-current="page" custom v-slot="{ navigate }">-->
            <div class="card text-white busqueda-card" @click="navigate">
                <img src="/images/Imagenes/alavaDescubre.jpg" class="card-img" alt="">
                <div class="card-img-overlay" v-bind:id="item.territory.split(' ')[0].split('/')[0]">
...
$(document).ready(function(){
   ...
   $('.h5-DocumentName').children('svg').click(function(){
